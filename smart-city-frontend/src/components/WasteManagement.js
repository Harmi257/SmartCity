...
import { fetchWasteStatus } from '../api'; // Update this import path based on your project

import 'chart.js/auto';
//import './WasteManagement.css'; // Optional: import your styles here
  ...
  return (
    <div className="waste-container">
      ...
      <div className="view-toggle">
        <button className={viewMode === 'progress' ? 'active' : ''} onClick={() => setViewMode('progress')}>
          Progress View
        </button>
      ...
      {viewMode === 'progress' ? (
          ...
          {wasteData.map((bin) => {
            ...
            return (
              <div key={bin.id} className="waste-card">
                ...
                <p className="status" style={{ color: getStatusColor(status) }}>{status}</p>
                ...
                {bin.level > 80 && (
                  scheduledBins.includes(bin.id) ? (
                    <>
                      ...
                      <div className="pickup-info">📦 A collection request has been scheduled. The team will be notified.</div>
                    </>
                  ) : (
                    <button className="pickup-btn" onClick={() => setScheduledBins([...scheduledBins, bin.id])}>
                      🧹 Schedule Pickup
                    </button>
                  )
                )}
        ...
        <>
          <div className="chart-toggle">
            <button className={chartType === 'type' ? 'active' : ''} onClick={() => setChartType('type')}>
              By Type
            </button>
